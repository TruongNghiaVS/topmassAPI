...
{
    public partial class CandidateAdminRepository : RepositoryBase<CampagnModel>, ICandidateAdminRepository
    {

        private readonly IJobRepository _jobRepository;
        public CandidateAdminRepository(IConfiguration configuration, IJobRepository jobRepository) : base(configuration)
        {
            _jobRepository = jobRepository;
        }
        public async Task<SearchCandidateAdminReponse> GetAll(dynamic request)
        {

            var result = await ExecuteSqlProcerduceToList<CandidateAdminItemDisplay>("sp_candidateAdmin_getall",
                request, System.Data.CommandType.StoredProcedure);
            var reponse = new SearchCandidateAdminReponse();
            reponse.Data = result;
            return reponse;
        }
        ...
        public async Task<CandidateAdminInfo> GetDetailBasic(int id)
        {
            var result = await ExecuteSqlProcerduceAndGetOan<CandidateAdminInfo>("sp_admin_candidate_getbasicInfo", new { id });
            return result;
        }
